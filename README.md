<h1 align="center">Ankush Gautam</h1>

<p align="center">
  Full-stack developer • Next.js • TypeScript • Python • Docker
</p>

<!-- Typing animation (fun personality touch) -->
<div align="center">
  <img 
    src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=20&pause=1000&color=cba6f7&center=true&vCenter=true&width=435&lines=When+life+gives+you+lemonade%2C+make+lemons...+%F0%9F%8D%8B" 
    alt="Typing SVG" />
</div>

<br>

<!-- Tech stack with gentle floating animation -->
<div align="center" style="position: relative; height: 80px; margin: 20px 0;">
  <div style="animation: float 6s ease-in-out infinite; position: absolute; left: 10%;">
    <img src="https://img.shields.io/badge/Next.js-000000?style=flat&logo=nextdotjs&logoColor=white&colorA=1e1e2e&colorB=cba6f7" alt="Next.js" />
  </div>
  <div style="animation: float 7s ease-in-out infinite 1s; position: absolute; left: 25%;">
    <img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white&colorA=1e1e2e&colorB=89b4fa" alt="TypeScript" />
  </div>
  <div style="animation: float 8s ease-in-out infinite 2s; position: absolute; left: 40%;">
    <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black&colorA=1e1e2e&colorB=f9e2af" alt="JavaScript" />
  </div>
  <div style="animation: float 9s ease-in-out infinite 3s; position: absolute; left: 55%;">
    <img src="https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white&colorA=1e1e2e&colorB=89dceb" alt="Python" />
  </div>
  <div style="animation: float 7.5s ease-in-out infinite 0.5s; position: absolute; left: 70%;">
    <img src="https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white&colorA=1e1e2e&colorB=74c7ec" alt="Docker" />
  </div>
  <div style="animation: float 8.5s ease-in-out infinite 1.5s; position: absolute; left: 85%;">
    <img src="https://img.shields.io/badge/Vercel-000000?style=flat&logo=vercel&logoColor=white&colorA=1e1e2e&colorB=b4befe" alt="Vercel" />
  </div>
</div>

<style>
@keyframes float {
  0%, 100% { transform: translateY(0); }
  50% { transform: translateY(-10px); }
}
</style>

### Focus
- Performant Next.js (App Router) web apps  
- Clean architecture & DX  
- Python services & scripting  
- Docker + modern workflows

### Currently
- Leveling up Next.js + TypeScript  
- Serverless / edge patterns  
- Open to interesting open-source collabs

<!-- Snake + styled contrib (crawling fun + Catppuccin pastel vibe) -->
<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://github.com/theankushgautam/theankushgautam/raw/output/github-snake-dark.svg" />
    <source media="(prefers-color-scheme: light)" srcset="https://github.com/theankushgautam/theankushgautam/raw/output/github-snake.svg" />
    <img alt="GitHub Contribution Snake" src="https://github.com/theankushgautam/theankushgautam/raw/output/github-snake.svg" width="80%" />
  </picture>
</div>

<!-- Activity stats (WakaTime-like flow in Catppuccin) -->
<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=theankushgautam&show_icons=true&theme=catppuccin_mocha&hide_border=true&hide_rank=true" alt="GitHub Stats" width="400" />
</div>

### Contact
📧 [theankushgautam@gmail.com](mailto:theankushgautam@gmail.com)

---
<p align="center"><small>When life gives you lemonade, make lemons.</small></p>
